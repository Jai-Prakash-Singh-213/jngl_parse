...
def page_for_seller(filename, prodetail):
    ...
    meta_title = str(meta_title.get_text())

    meta_disc = soup.find("meta", attrs={"name":"description"})
    meta_disc = str(meta_disc.get("content"))

    size = "None"
    size2 = soup.find("td", text=re.compile("Size"))

    try:
        size2 = size2.find_next_sibling("td")
        size = str(size2.get_text())
    except:
        pass

    desc = str(soup.find("div", attrs={"class":"descriptionheading"}))
    spec = str(soup.find("div", attrs={"class":"productDetailsTable"}))

    date = time.strftime("%d:%m:%Y")

    status  = "not defined"
    subproinfo = [meta_title, meta_disc, size, desc, spec, date, status]
 
    subproinfo = map(mystrip, subproinfo)
    ...
    if div_seller is not None:
        ...
        f.write(",".join(prodetail + procomp + subproinfo) + "\n")
        print  prodetail + procomp + subproinfo
	...
	procomp = [brand, whole_price, sellername, sellerlink, contact_phone, offerPrice, shippingPrice, seller_site]
        procomp = map(mystrip, procomp)
        #print >>f, ",".join(prodetail + procomp + subproinfo)
        f.write(",".join(prodetail + procomp + subproinfo) + "\n")
        print  prodetail + procomp + subproinfo
